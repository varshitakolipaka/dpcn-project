...
train_dataset = load_poisoned_cora(data, frac_poisoned=frac_poisoned, is_test=False, poison_tensor_size=poison_tensor_size, target_label=2)
...
# def run_ssd(model, dataset, adv_dataset, forget_mask, retain_mask, clean_test_data):
...
#     adv_accuracy_ssd = test_poisoned_data(unlearned_model, adv_dataset)
#     print(f'Poison success rate: {adv_accuracy_ssd:.4f}')
#     clean_accuracy_ssd = test_poisoned_data(unlearned_model, clean_test_data)
#     print(f'Clean Accuracy: {clean_accuracy_ssd:.4f}')

retain_mask = train_dataset.train_mask & ~train_dataset.poison_mask
# run_ssd(model=model, dataset=train_data, adv_dataset=adv_data, forget_mask=train_dataset.poison_mask, retain_mask=retain_mask, clean_test_data=clean_test_data)


def run_scrub(model, dataset, retain_mask, delete_mask, adv_dataset, clean_test_dataset=clean_test_dataset):
...
run_scrub(model=model, dataset=train_data, adv_dataset=adv_data, delete_mask=train_dataset.poison_mask, retain_mask=retain_mask, clean_test_dataset=clean_test_data)